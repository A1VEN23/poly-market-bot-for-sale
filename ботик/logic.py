...
class MetaculusStrategy:
    ...
    MIN_DIVERGENCE     = 0.15    # 15% gap minimum — tighter to avoid noise
    MIN_FORECASTERS    = 10      # Minimum forecasters — questions with <10 are unreliable
    MIN_OVERLAP        = 0.28    # Jaccard overlap threshold — prevents false keyword matches
    # If Meta probability is this extreme AND Polymarket disagrees strongly,
    # it almost always means the questions are NOT the same event.
    META_EXTREME_LOW   = 0.05    # Meta < 5% is suspicious when Poly > 20%
    META_EXTREME_HIGH  = 0.95    # Meta > 95% is suspicious when Poly < 80%
    ...
    async def _match_with_metaculus(
        ...
        for q in questions:
            ...
            if forecasters < self.MIN_FORECASTERS:
            ...
            #   +15 for overlap ≥ 0.45 (strong keyword match)
            #   +10 for forecasters ≥ 50
            #   +10 for forecasters ≥ 200
            #   divergence bonus: up to +15 (capped — divergence alone ≠ quality)
            base_conf = 55.0
            ...
            if forecasters >= 200:
                base_conf += 10.0
            div_bonus = min(divergence * 50, 15.0)   # max +15 from divergence
            confidence = min(base_conf + div_bonus, 85.0)  # hard cap 85%
    ...
    async def _ai_expert_signals(
        ...
        for market in markets[:25]:
            ...
            # Sane confidence: base 55 + divergence bonus (max +20), hard cap 80
            confidence = min(55.0 + divergence * 100, 80.0)
